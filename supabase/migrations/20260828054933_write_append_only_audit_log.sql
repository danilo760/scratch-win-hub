...
declare v_admin uuid:=auth.uid(); v_red redemptions%rowtype; v_before integer; v_after integer; v_previous text;
begin
 ...
 insert into audit_logs(admin_id,action,entity_type,entity_id,before_data,after_data,metadata) values(v_admin,'redemption.status_changed','redemption',v_red.id,jsonb_build_object('status',v_previous),jsonb_build_object('status',v_red.status),jsonb_build_object('protocol',v_red.protocol));
