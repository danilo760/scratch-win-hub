create or replace function public.get_active_scratchcards_v1()
returns table (
  id uuid,
  title text,
  price numeric,
  math_version_id uuid,
  rarity_slug text,
  rarity_name text,
  rarity_theme jsonb
)
...
as $function$
  select
    ...
    c.title,
    c.price,
    v.id as math_version_id,
    r.slug as rarity_slug,
    r.name as rarity_name,
    r.theme as rarity_theme
  from public.scratchcards c
  join lateral (
    select mv.id, mv.rarity_id
  ...
  where c.active = true
  order by c.price, c.id;
$function$;

revoke all on function public.get_active_scratchcards_v1() from public, anon;
grant execute on function public.get_active_scratchcards_v1() to authenticated;
