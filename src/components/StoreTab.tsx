...
export function StoreTab() {
  ...
  const redeem = async (id: string) => {
    setBusyId(id);
    const { data, error } = await supabase.rpc("redeem_item", { item_id_param: id });
    setBusyId(null);
    if (error) {
      toast.error(error.message);
      return;
    }
    const res = data as unknown as { success: boolean; new_points: number };
  ...
  if (!items?.length) {
    return <p className="py-16 text-center text-muted-foreground">Nenhum item disponível na loja.</p>;
  }
            ...
            <CardContent className="flex flex-1 flex-col justify-between gap-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant={noStock ? "destructive" : "secondary"} className="gap-1">
                  ...
                  {noStock ? "Esgotado" : `${item.stock} em estoque`}
                </Badge>
              </div>
              <Button
                variant="glow"
