...
import { Activity, AlertTriangle, BarChart3, Calculator, Loader2, RefreshCw, Users } from "lucide-react";
...
function parseMathConfig(value: unknown): MathConfig {
  ...
  const versions: MathVersion[] = list(raw.versions).flatMap((entry) => {
    const item = record(entry);
    if (!item || !str(item.id) || !str(item.scratchcard_id) || !str(item.version_name) || !str(item.status)) return [];
      ...
      <CardContent className="space-y-3">
        {rows.length ? rows.map((row) => (
          <div key={row.label} className="space-y-1">
            <div className="flex justify-between gap-3 text-sm"><span>{row.label}</span><strong>{row.count.toLocaleString("pt-BR")}</strong></div>
            <div className="h-2 overflow-hidden rounded bg-secondary">
              <div className="h-full rounded bg-primary" style={{ width: `${Math.max(2, (row.count / max) * 100)}%` }} />
            </div>
          </div>
...
export function AdminInsightPanel() {
  ...
  return (
    <Tabs defaultValue="operations" className="mb-6 space-y-4">
      <TabsList>
        <TabsTrigger value="operations"><BarChart3 className="mr-1 size-4" /> Operação</TabsTrigger>
        <TabsTrigger value="math-audit"><Calculator className="mr-1 size-4" /> Auditoria Matemática</TabsTrigger>
      </TabsList>
      <TabsContent value="operations"><OperationsPanel query={dashboardQuery} /></TabsContent>
      <TabsContent value="math-audit"><MathAuditPanel config={mathQuery.data ?? { cards: [], versions: [] }} loading={mathQuery.isLoading} /></TabsContent>
    </Tabs>
  );
}
...
function OperationsPanel({ query }: { query: ReturnType<typeof useQuery<AdminDashboard>> }) {
  ...
  if (query.error || !query.data) return <p role="alert" className="text-destructive">Não foi possível carregar os indicadores administrativos.</p>;
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Dia operacional: {query.data.local_date || "—"} · {query.data.timezone}</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => void query.refetch()}><RefreshCw className="size-4" /> Atualizar</Button>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi label="Jogadas hoje" value={cards.plays_today.toLocaleString("pt-BR")} />
        <Kpi label="Resultados premiados" value={`${cards.winning_results.toLocaleString("pt-BR")} (${winRate.toFixed(2)}%)`} />
        ...
        <Kpi label="Resgates em andamento" value={cards.pending_redemptions.toLocaleString("pt-BR")} warning={cards.pending_redemptions > 0} />
        <Kpi label="Itens com estoque baixo" value={cards.low_stock.toLocaleString("pt-BR")} warning={cards.low_stock > 0} />
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Activity className="size-4" /> Últimos 30 dias</CardTitle><CardDescription>Resumo diário em America/Sao_Paulo.</CardDescription></CardHeader>
        <CardContent className="overflow-x-auto">
          <table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b"><th className="p-2">Dia</th><th className="p-2">Jogadas</th><th className="p-2">Usuários ativos</th><th className="p-2">Pontos emitidos</th><th className="p-2">Pontos consumidos</th></tr></thead><tbody>{query.data.plays_by_day.map((day) => {
            const users = query.data.active_users_by_day.find((row) => row.day === day.day)?.count ?? 0;
            const points = query.data.points_by_day.find((row) => row.day === day.day);
            return <tr key={day.day} className="border-b"><td className="p-2">{new Date(`${day.day}T12:00:00`).toLocaleDateString("pt-BR")}</td><td className="p-2">{day.count}</td><td className="p-2">{users}</td><td className="p-2">{points?.issued ?? 0}</td><td className="p-2">{points?.consumed ?? 0}</td></tr>;
          })}</tbody></table>
        </CardContent>
      </Card>
    </div>
...
function MathAuditPanel({ config, loading }: { config: MathConfig; loading: boolean }) {
  const validVersions = useMemo(() => config.versions.filter((version) => version.outcomes.length > 0), [config.versions]);
  const [versionId, setVersionId] = useState("");
  useEffect(() => {
    if (!versionId && validVersions[0]) setVersionId(validVersions[0].id);
    if (versionId && !validVersions.some((version) => version.id === versionId)) setVersionId(validVersions[0]?.id ?? "");
  ...
  const auditQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_math_audit_v1" as never, { p_math_version_id: versionId } as never);
      if (error) throw error;
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Configurado × observado</CardTitle><CardDescription>Compara a probabilidade da versão com as jogadas realmente registradas nessa mesma versão.</CardDescription></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          <select className="h-10 min-w-[280px] flex-1 rounded-md border border-input bg-background px-3 text-sm" value={versionId} onChange={(event) => setVersionId(event.target.value)}>
            {validVersions.map((version) => <option key={version.id} value={version.id}>{config.cards.find((item) => item.id === version.scratchcard_id)?.title ?? "Raspadinha"} — {version.version_name} — {version.status}</option>)}
          </select>
          <Button variant="outline" onClick={() => void auditQuery.refetch()} disabled={!versionId || auditQuery.isFetching}><RefreshCw className={auditQuery.isFetching ? "size-4 animate-spin" : "size-4"} /> Atualizar</Button>
        </div>
        {selected && <div className="flex flex-wrap items-center gap-2 text-sm"><strong>{card?.title ?? "Raspadinha"}</strong><Badge variant={selected.status === "PUBLISHED" ? "default" : "secondary"}>{selected.status}</Badge><Badge variant="outline">{selected.rarity_name ?? selected.rarity_slug ?? "Sem raridade"}</Badge></div>}
        {auditQuery.error && <p role="alert" className="text-destructive">Não foi possível carregar a auditoria matemática.</p>}
        {auditQuery.data && <>
          <div className="flex items-center gap-2 rounded-lg border p-3 text-sm"><Users className="size-4" /><strong>{auditQuery.data.total_plays.toLocaleString("pt-BR")}</strong> jogadas observadas nesta versão.</div>
          {auditQuery.data.total_plays === 0 && <div className="flex items-start gap-2 rounded-lg border border-warning/40 bg-warning/5 p-3 text-sm"><AlertTriangle className="mt-0.5 size-4 text-warning" /><span>Sem amostra observada. A configuração pode ser inspecionada, mas ainda não há base estatística real para comparação.</span></div>}
          <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b"><th className="p-2">Resultado</th><th className="p-2">Configurado</th><th className="p-2">Observado</th><th className="p-2">Diferença</th><th className="p-2">Ocorrências</th><th className="p-2">Prêmio/Pontos</th></tr></thead><tbody>{auditQuery.data.outcomes.map((outcome) => {
            const diff = outcome.observed_percent - outcome.expected_percent;
            return <tr key={outcome.outcome_id} className="border-b"><td className="p-2 font-medium">{outcome.name}</td><td className="p-2">{outcome.expected_percent.toFixed(4)}%</td><td className="p-2">{outcome.observed_percent.toFixed(4)}%</td><td className="p-2">{diff >= 0 ? "+" : ""}{diff.toFixed(4)} pp</td><td className="p-2">{outcome.observed_count.toLocaleString("pt-BR")}</td><td className="p-2">{outcome.prize} cr · {outcome.points} pts</td></tr>;
          })}</tbody></table></div>
        </>}
      </CardContent>
    </Card>
